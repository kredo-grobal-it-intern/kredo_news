...
class UserController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $liked_news = $user->reactions()->with(['country', 'category', 'reactions'])->latest('published_at')->get()
            ->filter(function ($reaction) {
                return $reaction->pivot->status == 1;
            });
        $bookmarked_news = $user->bookmarks()->with(['country', 'category', 'reactions'])->latest('published_at')->get();
